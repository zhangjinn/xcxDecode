...
var index_4 = require('./../../../../components/empty-data-type/index.js');
var Filter = /** @class */ (function (_super) {
    ...
    function Filter() {
        ...
        _this.$props = { "noticeListItem": { "v-bind:item.sync": { "value": "item", "type": "item", "for": "msgList", "item": "item", "index": "index", "key": "key" } }, "filter": { "xmlns:v-bind": "", "v-bind:tabList.sync": "tabList", "v-bind:tabActive.sync": "tabActive", "xmlns:v-on": "" }, "emptyDataType": { "description": "通知" } };
        ...
        _this.methods = {
            ...
            onSwitch: function (item) {
                var currId = item.noticeType;
                var typeName = item.typeName;
                if (currId) {
                    var url = "/pages/me/message/detail/index?type=" + currId + "&typeName=" + typeName;
                    wx.navigateTo({
                        url: url
                    });
                }
    ...
    Filter.prototype.getMsgList = function () {
        ...
        this.methods.getNoticeNewList(data).then(function (res) {
            if (res && res.payload) {
                ...
                if (res.payload.content && res.payload.content.length) {
                    res.payload.content = res.payload.content.map(function (item) {
                        return __assign({}, item, { createDateStr: item.noticeDTO.createdDate ? index_3.formatDate(item.noticeDTO.createdDate, "Y-M-D") : '', content: item.noticeDTO.content, amount: terms.status == '0' ? item.noticeAmount : 0 });
                    });
                }
    ...
    Filter = __decorate([
        wepy_redux_1.connect({}, {
            getNoticeNewList: notice_1.getNoticeNewList,
        })
    ], Filter);
    return Filter;
}(wepy_1.default.page));

Page(require('./../../../../npm/wepy/lib/wepy.js').default.$createPage(Filter , 'pages/message/notice/list/index'));
