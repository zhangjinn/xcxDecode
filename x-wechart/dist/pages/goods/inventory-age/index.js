...
var InventoryOverTime = /** @class */ (function (_super) {
    ...
    function InventoryOverTime() {
        ...
        _this.methods = {
            ...
            getOrgList: function () {
                request_2.request({ api: "comm/queryOrg.nd?type=3" }).then(function (res) {
                    ...
                    if (_this.orgList && _this.orgList.length > 0) {
                        var defaultOrgId = _this.orgList[0].code;
                        _this.filterForm = __assign({}, _this.filterForm, { orgId: defaultOrgId });
                        _this.orgIds = [defaultOrgId];
                        _this.headerTabList[0].selectValue = _this.orgIds.join(',');
                    }
                    _this.inventoryList = [];
                    _this.filterForm = __assign({}, _this.filterForm, { pageNo: 1 });
                    _this.methods.getStockList();
                    _this.methods.getMatklList();
                    _this.methods.getOverTimeList();
                    _this.$apply();
                });
            },
            getStockList: function () {
                request_2.request({
                    api: "exceedStockList/getStockByOrgIdNewAll.nd", method: 'POST', data: {
                        orgId: _this.filterForm.orgId
                    }
                }).then(function (res) {
                    if (res && res.list) {
                        var tempArr = [];
                        tempArr = res.list.map(function (item) {
                            return {
                                key: item.code,
                                value: item.name,
                            };
            ...
            getmatnrNameList: function (e) {
                ...
                _this.matnrIds = [];
                // this.filterForm.matnr = ''
                _this.filterForm = __assign({}, _this.filterForm, { matnr: e.detail });
                // if(!this.matnrName){
                //   return
                // }
                // request(
            ...
            getOverTimeList: function () {
                ...
                request_2.request({
                    api: "exceedStockList/exceedInvageList.htm", method: 'POST', data: {
                        orgId: orgId,
                        stockId: stockId,
                    ...
                    _this.inventoryList = _this.inventoryList.concat(res.list);
    ...
    InventoryOverTime.prototype.onLoad = function (e) {
        var isWarning = e.isWarning;
        // 从消息通知跳转进来默认筛选在预警期内（是否预警/超期 = 预警与超期状态）的数据。
        if (isWarning && isWarning == 'YC') {
            this.filterForm = __assign({}, this.filterForm, { whetherToWarn: 'YC', whetherToWarnName: '预警与超期状态' });
        }
...
Page(require('./../../../npm/wepy/lib/wepy.js').default.$createPage(InventoryOverTime , 'pages/goods/inventory-age/index'));
