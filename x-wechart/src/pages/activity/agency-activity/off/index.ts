...
import { addNum, mulNum } from '@/utils/index'
...
export default class Filter extends wepy.page {
  ...
  data: Data = {
    ...
    formData:{
      ...
      totalInput: '', // 各项投入合计(万元)
      // activityReal: 0, // 活动实际销售额(万元)
      activityChannelReal: '', // 全渠道销售额
      activityShopReal: '', // 全门店销售额
      realTotalMoney: 0, // 实际投入金额(元)
      adCompany: '', // 待制作广告公司
      officeManager: { // 办事处经理
        id: '',
        name: ''
  ...
  methods = {
    ...
    onTabFilterFormChange(evt){
      ...
      if(key==='writeOffAmount'){
        let writeOffTotalAmount = 0
        this.tabInfoItem[this.tabActive].items.forEach((item)=>{
          writeOffTotalAmount = addNum(writeOffTotalAmount, item.writeOffAmount)
        })
        // 合计金额 = 每一项小计相加
        this.tabInfoItem[this.tabActive].totalAmount = writeOffTotalAmount
        return;
      }
      ...
      if(key==='realNum' || key==='realPrice'){
        // 实际小计 = 实际数量 * 实际单价
        let realNum = this.tabInfoItem[this.tabActive].items[index].realNum
        let realPrice = this.tabInfoItem[this.tabActive].items[index].realPrice
        this.tabInfoItem[this.tabActive].items[index].realTotal = mulNum(realNum, realPrice)

        let realTotalNum = 0
        let realTotalAmount = 0
        this.tabInfoItem[this.tabActive].items.forEach((item)=>{
          realTotalNum += Number(item.realNum)
          realTotalAmount = addNum(realTotalAmount, item.realTotal)
        })
        // 实际合计数量 = 每一项实际数量相加
        this.tabInfoItem[this.tabActive].realTotalNum = realTotalNum
        // 实际合计金额 = 每一项实际小计相加
        this.tabInfoItem[this.tabActive].realTotalAmount = realTotalAmount
    ...
    toAddStore: () => {
      ...
      if(checkResault){
        const { marketCenter, office, timeStart, agent, theme, place, target, distributor, totalInput, adCompany, officeManager, reason, experienceSharing, realTotalMoney, activityReal, startDate, endDate, activityChannelReal, activityShopReal } = this.data.formData
        ...
        let param = {
          ...
          realTotalMoney: realTotalMoney,//实际投入金额（可编辑）
          // activityReal: activityReal,//活动实际销售额（万元）（可编辑）
          applyStartTime: startDate, // 活动开始时间
          applyEndTime: endDate, // 活动结束时间
          activityChannelReal: activityChannelReal, // 全渠道销售额
          activityShopReal: activityShopReal, // 全门店销售额
          ...
          deleteAttachs: this.deleteAttachs,
        }
        if(!this.isClickable){
          return
        }
        ...
        this.methods.saveWriteFlowStart(param).then((res)=>{
          const { payload } = res
    ...
    checkParam: () => {
      const { marketCenter, office, timeStart, agent, theme, place, target, totalInput, adCompany, officeManager, reason, realTotalMoney, activityReal } = this.data.formData
      if (!marketCenter.id) {
        Toast.fail('请选择营销中心')
        return false
      }
    ...
    if(list && list.items &&  list.items.length>0){
      target = list.items.map((item)=>{
        return {
          ...
          type: item.type, // 种类
          realNum: item.realNum,//实际数量（可编辑）
          realPrice: item.realPrice,//实际价格（可编辑）
          realTotal: item.realTotal,//实际小计（可编辑）
        }
      })
    }
    return target
    ...
    if(type && type === 'category'){
      ...
      if(list && list.length>0){
        ...
        target.items.forEach((item)=>{
          target.totalAmount = addNum(target.totalAmount, item.writeOffAmount)
        })
        target.totalNum = target.items.length
      }
      return target
    }else{
      let target = {
      ...
      if(list && list.length>0){
        target.items = list.map((item)=>{
          return {
            ...
            remark: item.remark, // 备注
            total: item.total || 0, // 小计
            realNum: item.realNum || 0, // 实际数量
            realPrice: item.realPrice || 0, // 实际单价
            realTotal: item.realTotal || 0, // 实际小计
            type: item.type, // 种类
        ...
        target.items.forEach((item)=>{
          ...
          target.realTotalNum += Number(item.realNum)
          target.realTotalAmount = addNum(target.realTotalAmount, item.realTotal)
        })
      }
      return target
    }
    ...
    this.methods.getAgentActivityById(param).then((res)=>{
      ...
      if(data){
        ...
        this.formData = {
          ...
          activityChannelReal: detail.activityChannelReal, // 全渠道销售额
          activityShopReal: detail.activityShopReal, // 全门店销售额
          distributor: { // 参与分销商
