...
import { getGrossProfitRate } from '@/store/actions/home';
...
export default class extends wepy.page {
  ...
  methods = {
    ...
    getData:()=>{
      this.methods.getGrossProfitRate(this.filterForm).then(res1=>{
        ...
        if (result && result.mx) {
          result.mx.map(i => {
            dataArr.push({
              ...
              pickUpVolume: new Number(i.grossProfit||0).toFixed(1),
              proportion: (Number(i.prossProfitRate) * 100).toFixed(1)+'%'
            });
          });
        }
