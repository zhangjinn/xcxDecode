...
export default class InventoryOverTime extends wepy.page {
  ...
  methods = {
    ...
    getOrgList: () => {
      request({ api: `comm/queryOrg.nd?type=3` }).then(res => {
        ...
        if(this.orgList && this.orgList.length>0){
          let defaultOrgId = this.orgList[0].code
          this.filterForm = { ...this.filterForm, orgId:defaultOrgId }
          this.orgIds = [defaultOrgId]
          this.headerTabList[0].selectValue = this.orgIds.join(',')
        }
        this.inventoryList = [];
        this.filterForm = { ...this.filterForm, pageNo: 1 }
    ...
    getmatnrNameList: (e) => {
      ...
      this.matnrIds = []
      // this.filterForm.matnr = ''
      this.filterForm = { ...this.filterForm, matnr: e.detail }
    ...
    getOverTimeList: () => {
      ...
      request({
        ...
        this.inventoryList = this.inventoryList.concat(res.list)
  ...
  onLoad(e: { isWarning: any; }) {
    const { isWarning } = e

    // 从消息通知跳转进来默认筛选在预警期内（是否预警/超期 = 预警与超期状态）的数据。
    if(isWarning && isWarning == 'YC'){
      this.filterForm = { ...this.filterForm, whetherToWarn: 'YC', whetherToWarnName:'预警与超期状态'}
    }
