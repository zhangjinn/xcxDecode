...
import {getNormalSalesOrderCustomerInfo, submitChannelOrder, getCisPrice, getDeliveryMethod, getSystemParameters, getIsOpenSharedWarehouse, getWarehouseList} from '@/store/actions/dmsorder';
...
    }
    ...
    sendRequest: async (status: string) => {
      ...
      // 1、先请求总开关量
      // 2、渠道订单录入点击提交时，查询光伟的接口（入参：组织、物料组，返回值：Y、N）校验物料组是否开启共享
      //     如果开启共享，不允许从原仓发货
      ...
      if(systemParameters == 'Y'){
        let isOpenSharedWarehouseObj = await this.methods.getIsOpenSharedWarehouse({orgId: this.org.id, matklId: materialGroupCode})
